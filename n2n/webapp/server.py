...
from n2n.webapp.sessions import store
...
@app.post("/v1/redact")
async def redact(file: UploadFile, pack_id: str) -> dict:
    try:
        get_pack(pack_id)
    ...
    if report.status == "PASS_AUTO":
        token = store.create(output_path, manifest_path)
...
def download_output(token: str) -> FileResponse:
...
def download_manifest(token: str) -> FileResponse:
    session = store.get(token)
